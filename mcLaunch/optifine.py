...
import struct
from cache_system import mc_directory, get_version_list, wget, cached_content
...
class Patcher:
    ...
    @staticmethod
    def install_optifine_library(mc_version: str, of_edition: str, mc_lib_dir: str, source_file: str, watcher=Watcher):
        """
        ...
        base_file = os.path.join(mc_directory, f"versions/{mc_version}/{mc_version}.jar")
        if not os.path.exists(base_file):
            base_file = os.path.join(mc_directory, "launcher_cache", f"{mc_version}.jar")
            if not os.path.exists(base_file):
...
class Installer:
    ...
    def do_install(online_name, target_version_name=None, watcher=Watcher, context=None):
        ...
        Patcher.install_optifine_library(mc_ver, of_ed, str(context.libraries_dir),
                                         optifine_zip_path, watcher=watcher)
    ...
    def copy_minecraft_version(mc_ver, mc_ver_of, dir_mc_ver, watcher=Watcher):
        ...
        if not file_jar_mc.exists():
            ...
            mc_version_url = get_version_list()[mc_ver]["url"]
            mc_jar_url = requests.get(mc_version_url).json()["downloads"]["client"]["url"]
            base_file = os.path.join(mc_directory, "launcher_cache", f"{mc_ver}.jar")
            wget(mc_jar_url, base_file, watcher)
            file_jar_mc = base_file
    ...
    @staticmethod
    def take_official_json(version):
        json_default_path = os.path.join(mc_directory, "versions", version, version + ".json")
        if os.path.exists(json_default_path):
            with open(json_default_path, "r") as f:
                return json.load(f)
        else:
            json_url = get_version_list()[version]["url"]
